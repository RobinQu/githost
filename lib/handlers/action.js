...
module.exports = function *() {
  ...
  var tmp = through();
  var decoder = encodings[this.get('content-encoding')] || encodings[this.get('content-encoding')]();
  if(decoder) {
  ...
  yield git.delegate.startPs(psEvent);
  ...
  var reply = through(function (c, enc, callback) {
    if (c.length === 4 && c.toString() === '0000') {//delay the signal for ending
      git.delegate.message(git, {type: 'reply', repo: this.repo}, write(this));
      callback();
    } else {
      debug('pass');
      callback(null, c);
    ...
    var self = this;
    git.delegate.after(git, psEvent, write(this), function () {
      debug('reply ends');
